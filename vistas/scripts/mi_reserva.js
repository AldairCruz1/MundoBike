...
function detalles(id_reserva){
  $.post("../../controladores/reserva.php?op=listadodetalles3", {
        id_reserva: id_reserva
      }, function (response) {
...
function agregar(id_bicicleta) {
  if (copy != "" || copy == "inicio") {
    ...
    if (hora_inicio == "" || hora_fin == "") {
        ...
        $.post("../../controladores/reserva.php?op=validar", {
            ...
            if (response.condicion == 0) {
                ...
                if(bicicletas.length == 1){
                    ...
                    $("#btnAgregarBicicleta").html('<label>Bicicleta:</label>');
                ...
                if(copy != "inicio"){
                    ...
                    id_copy = copy.attr("id").split("-");

                    id_copy_bicicleta = id_copy[0];

                    $("#customCheckbox" + id_copy_bicicleta).removeAttr("checked");
                    $("#delete" + id_copy_bicicleta).attr("class", "btn btn-sm bg-teal");
                    $("#delete" + id_copy_bicicleta).attr("onclick", "agregar('" + id_copy_bicicleta + "')");
                    $("#delete" + id_copy_bicicleta).html("<i class ='fas fa-check-circle '> </i> Seleccionar");
                    $("#delete" + id_copy_bicicleta).attr("id", "add" + id_copy_bicicleta);

                    copia2 = $("#" + id_copy_bicicleta + "-contend").clone();

                    $("#" + id_copy_bicicleta + "-contend").remove();

                    $("#listado-detalles").append(copia2);
      ...
      Toast.fire({
          icon: 'error',
          title: 'Seleccione el intercambio!!'
      });
  }
...
//   $("#delete" + id_bicicleta).html("<i class ='fas fa-plus '> </i> Seleccionar");
//   $("#delete" + id_bicicleta).attr("id", "add" + id_bicicleta);
  copy = $("#" + id_bicicleta + "-contend").clone();
  $("#detalles").show();
  $("#form").hide();
    ...
    if (hora_inicio != "" && hora_fin != "") {
      if (hora_inicio >= hora_fin) {
        ...
        if (bicicletas.length > 0) {
          ...
          $("#btnAgregarBicicleta").html('<label>Bicicleta:</label> <button id="btn-detalles" class="btn btn-xs btn-warning text-white mx-2" type="button" onclick="mostrar_bicicletas()"><i class="fa fa-list"></i> Ver Bicicletas </button>');
